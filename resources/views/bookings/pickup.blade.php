...
<div class="inspection-container">
    ...
    <div class="inspection-card">
        <div style="margin-bottom: 0.5rem;">
            <strong>Pickup Location:</strong> {{ $booking->pickup_point ?? 'Not specified' }}
        </div>
        <div>
            <strong>Pickup Date:</strong> {{ \Carbon\Carbon::parse($booking->rental_start_date)->format('d M Y, h:i A') }}
        </div>
    </div>
    ...
    @if(session('success'))
        <div style="background: #d1fae5; border: 1px solid #10b981; border-radius: 8px; padding: 1rem; margin-bottom: 1.5rem;">
            <strong style="color: #059669;">Success:</strong> {{ session('success') }}
        </div>
    @endif

    <form method="POST" action="{{ route('pickup.confirm', $booking) }}" id="inspectionForm" enctype="multipart/form-data">
        @csrf
